...
df_countries.to_sql('countries', engine, index=False, index_label="pk", if_exists="replace")
...
df_records[["lim_edition", "number", "remarks"]] = df_records[["lim_edition", "number", "remarks"]].fillna("")
...
df_1["lim_edition"] = df_1.apply(lambda x: limited(x["number"], x["lim_edition"]), axis=1)
...
df_3["credit_value"] = np.where(
    (df_3["record_format_id"].isin([3, 4, 5, 7, 8, 9, 10]) & (df_3["is_active"] == True)),
    1,
    0
)
...
pd.read_sql(
    "SELECT * FROM discobase_recordformat",
    engine_pg
)
...
pd.read_sql(
    "SELECT * FROM discobase_trxcredit",
    engine_pg
)
...
pd.read_sql(
    "SELECT * FROM discobase_record_artists",
    engine_pg
)
...
def migrate_data_without_id(engine_pg, engine_sqlite, table_pg, table_sqlite, cols_to_rename):
    # Read sqlite into df
    df = pd.read_sql(
        f"SELECT * FROM {table_sqlite}",
        engine_sqlite
